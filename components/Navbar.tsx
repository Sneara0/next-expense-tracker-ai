      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex items-center justify-between h-14 sm:h-16'>
          ...
          >
            ...
            <span className='text-sm sm:text-base md:text-lg lg:text-xl font-bold bg-gradient-to-r from-emerald-600 via-green-500 to-teal-500 bg-clip-text text-transparent'>
              <span className='hidden sm:inline animate-bounce'>ExpenseTracker AI</span>
          ...
          <div className='flex items-center space-x-1 sm:space-x-2'>
            ...
            <button
              ...
              className={`md:hidden p-2 rounded-full transition-all duration-300 ${
                isMobileMenuOpen ? 'animate-bounce text-pink-500' : 'animate-pulse text-blue-900 rounded-full hover:bg-pink-500 dark:hover:bg-blue-700'
              }`}
              aria-label='Toggle mobile menu'
            >
              <svg
                className={`w-6 h-6 transition-transform duration-300 ${
                  isMobileMenuOpen ? 'rotate-90 scale-110' : 'scale-100'
                }`}
                fill='none'
                stroke='currentColor'
                viewBox='0 0 24 24'
        ...
        <div
          className={`md:hidden transform transition-all duration-500 ease-in-out ${
            isMobileMenuOpen ? 'max-h-96 opacity-100 pb-3 animate-fadeInDown' : 'max-h-0 opacity-0 overflow-hidden'
          }`}
        >
